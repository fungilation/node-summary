...
function splitContentToSentences(content, callback) {
	...
	if (NativeModules?.SwiftSummary?.splitContentToSentencesCB)
		NativeModules.SwiftSummary.splitContentToSentencesCB(content, (sentences) => {
			// console.log('>>> node-summary > NativeModules.SwiftSummary.splitContentToSentencesCB > sentences:', sentences)
			callback(sentences || [])
		})
	else
	...
	return new Promise((resolve, reject) => {
		splitContentToSentences(paragraph, function(sentences) {
			if (!sentences || sentences.length < 2) {
				// reject(new Error(`No valid sentences. Length: ${sentences.length}`))
				return
			}
...
function getSentencesRanks(content, callback, sentences_dict) {
	...
	splitContentToSentences(content, function(sentences) {
		var n          = sentences.length,
				...
				nRange     = _.range(n)
		const sleepFreq = Math.round(240 / n) // 1000 / 42 (microsecs in SwiftSumary) = 23.81 ~ 24. So: 24 / n * sleepTime, or sleep(10) below
		...
		_.each(zeroNRange, async function(i) {
			...
			if (i % sleepFreq === 0)
				await sleep(16) // 10 before. Slowdown on large matrix math, smaller ones below sentencesRanksTextLimit delegated to SwiftSummary
		})
...
// title: NoNative to use JS instead of SwiftSummary
// sentences_dict: int as override for chopping content sent to expensive getSentencesRanks()
exports.summarize = function(title, content, callback, sentences_dict) {
	var sentencesRanksTextLimit = 50000 // 20000 would be decently fast on iPhone X
	if (_.isInteger(sentences_dict)) {
		sentencesRanksTextLimit = sentences_dict
		sentences_dict = undefined
	}
	if (content.length < sentencesRanksTextLimit && NativeModules?.SwiftSummary?.getSummary && title != 'NoNative') {
		NativeModules.SwiftSummary.getSummary(title, content, (summary, dict) => {
			// console.log('> node-summary > NativeModules.SwiftSummary.getSummary > summary, dict:', summary, dict)
			callback(false, summary, dict)
		})
	}
	else {
		if (title == 'NoNative') title = ''
		var summary = [], paragraphs = [], err = false
		getSentencesRanks(content.substr(0, sentencesRanksTextLimit), function(dict) {
			splitContentToParagraphs(content, async function(paragraphs) {
				summary.push(title) // Store the title.

				_.each(paragraphs, async function(p) {
					const sentence = await getBestSentence(p, dict)
					if (sentence) summary.push(sentence)
					// console.log('sentence:', sentence)
				})
				// HACKFIX: otherwise, can't guarantee above doesn't get run in async until after callback below sent. 100ms barely for long wikipedia articles
				if (summary.length <= 2) {
					await sleep(50)
					if (summary.length <= 2) {
						await sleep(200)
						if (summary.length <= 2) await sleep(800)
					}
				}

				// If we only have a title, then there is an issue.
				if (summary.length <= 2) err = true
				// console.log('callback:', err, `summary: ${summary.join("\n")}`, summary, 'dict:', dict, content)
